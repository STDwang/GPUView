...
版本：v0.2 Widgets分析演示。新增 [P0/P1真实帧、异步统计和缓存验证案例](P0_P1_WALKTHROUGH.md)。以下区分“已实现并验证”“工程情景”“尚未接入”，不将为了说明而构造的情景写成生产事故。问题原文见 [十题清单](QUESTIONS.md)。
...
PresentMon v1字段适配、无效行报告、异步范围统计与离屏CPU交互基准已补入P0/P1案例。多源对齐、通用版本适配、完整回放背压和屏幕实际呈现基准仍未实现；不能把已有闭环当完整Profiler。

2026-09-19：Q01/Q02/Q04/Q05/Q06/Q10新增[帧明细、热力图与原子导出案例](FRAME_ANALYSIS_WALKTHROUGH.md)，含复现、具体代码、测试、测量与练习。

全量中文注释及按数据流阅读顺序见[源码阅读入口](CODE_READING.md)，可配合本案例中的函数定位和测试进行口述练习。
