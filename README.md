...
当前是 **v0.2 Widgets 分析演示版**：百万教学事件时间线、精确选区统计，以及真实 PresentMon v1 CSV 的导入、帧间隔曲线和长帧定位已贯通。支持轨道过滤/折叠、事件高亮、视图历史、后台取消与失败保留会话。**仍不是完整 GPU Profiler：没有真实 Kernel、指令或调用栈分析，热力图、资源遥测、回放、导出和 Quick 对照尚未实现。**
...
统计页选择进程/交换链，双击长帧行或点击“定位最长事件 / 帧”；框选重新计算当前显示数据，缩放后可返回上一视图。来源页保留导入警告。
...
1. **A后续**：帧热力图、完整帧明细表、导出与干净环境交付。导入、分组统计、长帧联动已有实现。
